...
public class LanchoneteController {
    ...
    public ResponseEntity<Lanchonete> incluir(@Valid @RequestBody Lanchonete lanchonete) {
        ...
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(lanchonete.getId()).toUri();
        return ResponseEntity.created(location).body(lanchonete);
    }

    @GetMapping("{id}/detalhes")
    public ResponseEntity<LanchoneteResponse> obterDetalhes(@PathVariable Long id) {
        LanchoneteResponse lanchoneteResponse = lanchoneteService.obterDetalhes(id);
        return ResponseEntity.ok(lanchoneteResponse);
    }

    @PostMapping("/{lanchoneteId}/entregas/{entregaId}")
    public ResponseEntity<LanchoneteResponse> matricularEntrega(@PathVariable Long lanchoneteId, @PathVariable Long entregaId) {
        LanchoneteResponse lanchoneteResponse = lanchoneteService.matricularEntrega(lanchoneteId, entregaId);
        return ResponseEntity.ok(lanchoneteResponse);
    }
